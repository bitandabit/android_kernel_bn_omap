		...
		u8 *data, int len)
{
	...
	if (len == 1) {
		BUG();
		r = dsi_vc_send_short(dsidev, channel, MIPI_DSI_DCS_SHORT_WRITE,
				data[0], 0);
	} else if (len == 2) {
		BUG();
		r = dsi_vc_send_short(dsidev, channel, MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				data[0] | (data[1] << 8), 0);
	} else {
		r = dsi_vc_send_long(dsidev, channel, MIPI_DSI_GENERIC_LONG_WRITE,
				data, len, 0);
	}
			...
			int x, int y, int w, int h)
{
	...
	while (pixels_left > 0) {
		...
		dsi_vc_write_long_header(dsidev, 0, MIPI_DSI_DCS_LONG_WRITE,
				1 + pixels * bytespp, 0);
